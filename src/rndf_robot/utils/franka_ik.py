...
class FrankaIK:
    def __init__(self, gui=True, base_pos=[0, 0, 1], external_pb_client=None):
        if gui:
            print('SETTING RENDERER INSIDE OF FRANKAIK')
            # set_client(0)
            set_client(1)
        else:
            set_client(1)
        ...
        # self._grasp_target_to_ee = [0, 0, -0.0, 0, 0, 0, 1]
        # self._ee_to_grasp_target = [0, 0, 0.0, 0, 0, 0, 1]

    def set_jpos(self, jnts):
        set_joint_positions(self.robot, self.ik_joints, jnts)

    def set_base_position_and_orientation(self, body_id, pos, ori):
    ...
    def plan_joint_motion(self, start, goal, alg='rrt_star', max_time=10.0):
        self.set_jpos(start)
        plan = plan_joint_motion(
            self.robot, self.ik_joints, goal, obstacles=self.obstacle_dict.values(), attachments=self.attachment_dict.values(), 
            self_collisions=True, disabled_collisions=set(self.panda_ignore_pairs), algorithm=alg, max_time=max_time)
        return plan
