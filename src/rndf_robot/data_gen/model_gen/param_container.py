...
def make_container(bl, bw, bt, wt, wh, th, show=False, mc_vis=None):
    """
    Args:
    ...
    front_rot = R.from_euler('xyz', [0, (np.pi/2 + fb_theta), 0]).as_matrix()
    ...
    back_rot = R.from_euler('xyz', [0, -(np.pi/2 + fb_theta), 0]).as_matrix()
    ...
    left_rot = R.from_euler('xyz', [-(np.pi/2 + side_theta), 0, 0]).as_matrix()
    ...
    right_rot = R.from_euler('xyz', [(np.pi/2 + side_theta), 0, 0]).as_matrix()
    ...
    for k, v in cones.items():
        if k == 'fl':
            rot = R.from_euler('xyz', [(np.pi - fb_theta), (side_theta), 0]).as_matrix()
    ...
    if show:
        assert mc_vis is not None, 'Cannot show without meshcat visualizer!'
