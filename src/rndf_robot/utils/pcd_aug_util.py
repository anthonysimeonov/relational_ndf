...
def sample_cam_pose_mat(focus_pt=None):
    ...
    # norm = focus_pt / np.linalg.norm(focus_pt)

    cam_pose_mat = np.eye(4)
    # cam_pose_mat[:-1, :-1] = R.from_euler('xyz', [0, -1.0*pitch, yaw]).as_matrix()
    cam_pose_mat[:-1, :-1] = R.from_euler('xyz', [roll, -1.0*pitch, 0]).as_matrix()
    # cam_pose_mat[:-1, :-1] = R.from_euler('xyz', [0, pitch, yaw - np.pi/2]).as_matrix()
...
def simulate_random_occlusions(pcd, cam_poses, cam_int_mat, img_size=(480, 640), select_prob=1.0):
    ...
    for cam_idx in range(n_cams):
        ...
        # round the x, y indices, and keep the pixel with the minimum z value
        # bad way to do it -- will vectorize
        for ii in range(depth_int.shape[0]):
            # get the pixel coordinate
            v, u = depth_int[ii, :-1].astype(int)

            if u >= final_depth.shape[0] or v >= final_depth.shape[0]:
                continue

            if u < 0 or v < 0:
                continue

            # if u >= final_depth.shape[0]:
            #     print('larger u')
            #     from IPython import embed; embed()
            #     u = final_depth.shape[0] - 1
            # if v >= final_depth.shape[1]:
            #     print('larger v')
            #     from IPython import embed; embed()
            #     v = final_depth.shape[1] - 1

            # print('u, v, ii', u, v, ii)
            if final_depth[u, v] == 0.0 or (depth_int[ii, -1] < final_depth[u, v]):
                final_depth[u, v] = depth_int[ii, -1]
...
def pcd_aug_full(pcd, rot_grid, deform_about_point=None, rnd_occlusion=True, cut_plane=True, 
    ...
    pcd_aug = pcd.copy()
    st = time.time()
    # TODO: create way to simulate different cameras
    cam_poses = []
    for i in range(4):
        cam_pose_mat = sample_cam_pose_mat(focus_pt=np.mean(pcd, axis=0))
        # cam = simulate_camera(cam, cam_int_mat, cam_pose_mat)
    ...
    if rnd_occlusion:
        pcd_aug = simulate_random_occlusions(pcd, cam_poses, cam_int_mat)
    ...
    if per_point_noise:
        ...
        pt_noise = np.random.random(size=pcd_aug.shape) * per_point_noise_std
